...
public sealed class FirebaseFileStorage(
    ...
    FirebasePathResolver pathResolver,
    FirebaseSignedUrlGenerator signedUrlGenerator,
    IContentTypeProvider contentTypeProvider,
    IOptions<FirebaseStorageOptions> options
) : IFileStorageProvider
{
    private const string FirebaseDownloadTokenMetadataKey = "firebaseStorageDownloadTokens";
    ...
    )
    {
        ...
        var firebaseDownloadToken = options.Value.UseFirebaseDownloadToken
            ? Guid.NewGuid().ToString("D")
            : null;

        var metadata = BuildMetadata(file.Metadata, firebaseDownloadToken);
        ...
        return new StoredFile
        {
            ...
            Path = objectName,
            Url = BuildUrl(objectName, visibility, firebaseDownloadToken),
            ContentType = contentType,
            SizeInBytes = size,
    ...
    private string? BuildUrl(
        string objectName,
        FileVisibility visibility,
        string? firebaseDownloadToken
    )
    {
        if (
            visibility == FileVisibility.Private
            && string.IsNullOrWhiteSpace(firebaseDownloadToken)
        )
        {
            return null;
        }

        if (!options.Value.UsePublicUrl && string.IsNullOrWhiteSpace(firebaseDownloadToken))
        {
            return null;
        }

        return pathResolver.ResolvePublicUrl(objectName, firebaseDownloadToken);
    }
    ...
    )
    {
        ...
        {
            result[FirebaseDownloadTokenMetadataKey] = firebaseDownloadToken;
        }
